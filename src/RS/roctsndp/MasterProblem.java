...
public class MasterProblem {
	...
	public GRBVar epsilon;

	public GRBLinExpr obj;
	public GRBLinExpr cutbase;
	public GRBLinExpr cutbase1;
	...
	public ArrayList<ArrayList<ArrayList<GRBVar>>> new_w;//decision variable s
	...
	public int epsilon_scale = 1;
	public int scale_size = 0;
	...
	public void ConsMaterProblem(FlatG flatg, double taskobj) throws GRBException {
		...
		epsilon = model.addVar(0, GRB.INFINITY, 0.0, GRB.CONTINUOUS, "epsilon");
		
		//constraints
		GRBLinExpr expr1;//flow balance
		GRBLinExpr expr2;//capacity
		GRBLinExpr expr2_1;//capacity
		GRBLinExpr expr4;//sigma asymmetric
		...
		GRBLinExpr expr20;//(cx+fy+hcost)norminal -taskobj<= 0
		
		int size = 0;
		int node = 0;
		int index = 0;
		cutbase =  new GRBLinExpr();
		cutbase1 =  new GRBLinExpr();
		expr20 =  new GRBLinExpr();
		for(int k=0; k<commo_num; k++) {
			...
			for(int i=0; i<term_num; i++) {
				...
				for(int j=0; j<size; j++) {
					...
					for(int r=0; r<flatg.coso_commodity.get(i).get(j).size(); r++) {
						z.get(k).get(i).get(j).add(model.addVar(0, 1, 0.0, GRB.BINARY, "sigma" + i+","+ node +"," + k+","+r));					
						if(com_index < flatg.scale_conflict.get(i).get(j)) {
							if(r != com_index) {
			...
			for(int i=0; i<term_num; i++) {
				...
				if(expr19.size() > 1) {
					...
					expr20.addTerm(flatg.C_holdcost[k][i], w.get(k).get(i));
					cutbase1.addTerm(flatg.C_holdcost[k][i], w.get(k).get(i));
				}
			}
		}
		GRBLinExpr expr18;
		...
		expr20.add(cutbase);
		cutbase1.add(cutbase);
		expr20.addTerm(-1, epsilon);
		model.addConstr(expr20, GRB.LESS_EQUAL, taskobj, "c" + constrain_num);
		needmodify = constrain_num;
		constrain_num++;
		model.setObjective(obj, GRB.MINIMIZE);//minimize the objective*/
		model.write("MPout.lp");
	}
	
	public void changetheconstraint(FlatG flatg, double taskobj, double UB_value) throws GRBException {	
		GRBConstr c = model.getConstrByName("c" + needmodify);
		model.remove(c);
		GRBLinExpr expre = new GRBLinExpr();
		expre.add(cutbase1);
		model.addConstr(expre, GRB.LESS_EQUAL, taskobj, "c" + constrain_num);
		constrain_num++;
		...
		while(UB > 10000) {
			UB = UB/10;
			epsilon_scale = epsilon_scale*10;
		}
		System.out.println("epsilon_scale " + epsilon_scale);
		obj = new GRBLinExpr();
		obj.addTerm(epsilon_scale, epsilon);
		model.setObjective(obj, GRB.MINIMIZE);//minimize the objective*/
	}
	public void addLBeq(double LB_value) throws GRBException {	
		GRBConstr c = model.getConstrByName("c" + LBeq);
		model.remove(c);
		GRBLinExpr expr = new GRBLinExpr();
		expr.addTerm(epsilon_scale, epsilon);
		model.addConstr(expr, GRB.GREATER_EQUAL, LB_value, "c" + constrain_num);
		LBeq = constrain_num;
		constrain_num++;
	}
	...
	public void addvaricons(FlatG flatg, int worstindex) throws GRBException {
		...
		expr8_1.add(cutbase);
		expr8_1.addConstant(-flatg.taskobj);
		expr8_1.addTerm(-flatg.trav_time_worst_Gamma.get(worstindex)*epsilon_scale, epsilon);
		...
		for(int k=0; k<commo_num; k++) {
			for(int i=0; i<term_num; i++) {
				...
				for(int j=0; j<size; j++) {
					...
					for(int r=0; r<flatg.coso_commodity.get(i).get(j).size(); r++) {
						if(MP.z.get(k).get(i) != null) {
							if(MP.z.get(k).get(i).get(j)!= null) {
								if(MP.z.get(k).get(i).get(j).get(r) != null) {
	...
	public void add_cut_2(FlatG flatg, ArrayList<ArrayList<ArrayList<Integer>>> x_value, ArrayList<ArrayList<ArrayList<ArrayList<Integer>>>> sigma_value) throws GRBException {
		int size = 0;
		int node = 0;
		...
		for(int k=0; k<commo_num; k++) {
			for(int i=0; i<term_num; i++) {	
				...
				for(int j=0; j<size; j++) {
					...
					node = flatg.fadj_matrix.get(i).get(j);
					for(int r=0;r<flatg.r_scale_s[i][node]; r++) {
						if(sigma_value.get(k).get(i).get(j).get(r) == 1) {
	...
	public void add_cut_1(FlatG flatg, double rs_index, ArrayList<ArrayList<ArrayList<Integer>>> x_value, ArrayList<ArrayList<ArrayList<ArrayList<Integer>>>> sigma_value) throws GRBException {
		int size = 0;
		int node = 0;
		GRBLinExpr exprcut = new GRBLinExpr();
		exprcut.addTerm(-1, epsilon);
		for(int k=0; k<commo_num; k++) {
			for(int i=0; i<term_num; i++) {	
				if(x_value.get(k).get(i) == null) {
					continue;
				...
				for(int j=0; j<size; j++) {
					...
					if(x_value.get(k).get(i).get(j) == 1) {
						exprcut.addTerm(Math.ceil(rs_index), x.get(k).get(i).get(j));
						exprcut.addConstant(-Math.ceil(rs_index));
					}
					node = flatg.fadj_matrix.get(i).get(j);
					for(int r=0;r<flatg.r_scale_s[i][node]; r++) {
						if(sigma_value.get(k).get(i).get(j).get(r) == 1) {
							exprcut.addTerm(Math.ceil(rs_index), z.get(k).get(i).get(j).get(r));
							exprcut.addConstant(-Math.ceil(rs_index));
						}
					}
				}
	...
	public boolean generfinalsolu_i(FlatG flatg, int soluindex, double UB_soluvalue, double record_UB, boolean conti, MasterProblem MP, int soluindex1) throws GRBException, IOException {
		...
		ArrayList<ArrayList<ArrayList<ArrayList<Integer>>>> sigmasolu = new ArrayList<ArrayList<ArrayList<ArrayList<Integer>>>>();
		...
		for(int k=0; k<commo_num; k++) {
			...
			sigmasolu.add(new ArrayList<ArrayList<ArrayList<Integer>>>());
			for(int i=0; i<term_num; i++) {	
				if(x.get(k).get(i) == null) {
					xsolu.get(k).add(null);
					sigmasolu.get(k).add(null);
					continue;
				}
				xsolu.get(k).add(new ArrayList<Integer>());
				sigmasolu.get(k).add(new ArrayList<ArrayList<Integer>>());
				size = flatg.fadj_matrix.get(i).size();
				for(int j=0; j<size; j++) {
					if(x.get(k).get(i).get(j)== null) {
						xsolu.get(k).get(i).add(null);
						sigmasolu.get(k).get(i).add(null);
						continue;
					}
					xsolu.get(k).get(i).add(0);
					sigmasolu.get(k).get(i).add(new ArrayList<Integer>());
					for(int r=0;r<flatg.r_scale_s[i][flatg.fadj_matrix.get(i).get(j)]; r++) {
						sigmasolu.get(k).get(i).get(j).add(0);
					}
				}
			}
		}
							...
							for(int r=0;r<MP.z.get(k).get(nodenow).get(j).size(); r++) {
								if(MP.z.get(k).get(nodenow).get(j).get(r) != null) {
									if(Double.compare(MP.z.get(k).get(nodenow).get(j).get(r).get(GRB.DoubleAttr.Xn), 0.9) > 0) {
										sigmasolu.get(k).get(nodenow).get(j).set(r, 1);
										Arcrc ac = new Arcrc();
										ac.fromnode = nodenow;
										ac.tonode = nodenext;
		...
		solution = xsolu.toString()+"s"+sigmasolu.toString();
		double demand = 0;
		...
		if(soluindex1 == 0) {
			...
			obtained_MPz_value.add(sigmasolu);
	...
	public void Detergenerfianlsolu_short(FlatG flatg) throws GRBException, IOException {
		...
		//generate x solution and sigma solution
		int conindex = 0;
		ArrayList<ArrayList<Arcrc>> commosolu = new ArrayList<ArrayList<Arcrc>>();
		for(int k=0; k<commo_num; k++) {
			commosolu.add(new ArrayList<Arcrc>());
			for(int i=0; i<flatg.shortestpath.get(k).size()-1; i++) {
				...
				sigmasolu.get(k).get(nodenow).get(nodeindex).set(conindex, 1);
			}
		}
	...
	public void Detergenerinisolu(FlatG flatg) throws GRBException, IOException {
		...
		for(int k=0; k<commo_num; k++) {
			...
			for(int i=0; i<flatg.inisolu.get(k).size()-1; i++) {
				...
				int com_index = flatg.coso_commodity.get(nodenow).get(nodeindex).indexOf(k);
				sigmasolu.get(k).get(nodenow).get(nodeindex).set(com_index, 1);
