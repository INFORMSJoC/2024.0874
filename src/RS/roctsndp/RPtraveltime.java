...
public class RPtraveltime {
	...
	public boolean positiveG;
	public boolean feasibleM;
	...
	public void scale_b(MasterProblem MP, ArrayList<ArrayList<Integer>> commoso) {
		int size = 0;
		int scale_size = 1;
		...
		MP.scale_size = scale_size;
		bigM = scale_size*bigM_0;
		System.out.println("scale_size " + scale_size +" bigM " + bigM);
	}
	
	public void ConsSubProblem(FlatG flatg, MasterProblem MP, ArrayList<ArrayList<ArrayList<Integer>>> x, ArrayList<ArrayList<ArrayList<ArrayList<Integer>>>> z, ArrayList<Arcrc> travelarcs, double rs_index, boolean[][] travelornot, ArrayList<ArrayList<Integer>> commoso, double MPobjvalue) throws GRBException{
		if(model != null) {
			clear1();
		}
		...
		GRBLinExpr feasiblitycut = new GRBLinExpr();
		feasiblitycut.addConstant(MPobjvalue);
		...
		for(int k=0; k<commo_num; k++) {
			 ...
			 */
			psi.add(model.addVar(0, bigM_0, 0.0, GRB.CONTINUOUS, "psi"+k));
			feasiblitycut.addTerm(-flatg.C_due_time[k], psi.get(k));
			gamma.add(model.addVar(0, bigM_0, 0.0, GRB.CONTINUOUS, "gamma"+k));
			feasiblitycut.addTerm(flatg.C_EValia_time[k], gamma.get(k));
			...
			for(int i=0; i<term_num; i++) {	
				if(x.get(k).get(i) == null) {
					...
					model.addConstr(expr4, GRB.LESS_EQUAL, flatg.C_holdcost[k][i]*1000, "c" + constrain_num);
					constrain_num++;
					if(i == flatg.C_origin.get(k)) {
						feasiblitycut.addTerm(-flatg.C_EValia_time[k], lambda.get(k).get(i));
						beta.get(k).add(null);
			...
			model.addConstr(expr3, GRB.LESS_EQUAL, flatg.delaypenalty[k]*1000, "c" + constrain_num);
			constrain_num++;
		}
		GRBLinExpr expre;
		GRBLinExpr expre1;
				...
				for(int r=0; r<flatg.r_scale_s[i][node]; r++) {
					Arcrc newarc = new Arcrc();
 					...
 					if(index != -1) {
								...
								if(z.get(commodity).get(i).get(j) != null){
									if(z.get(commodity).get(i).get(j).get(r) == 1){
										if(node != flatg.C_origin.get(commodity) && node != flatg.C_destination.get(commodity)) {
											...
											feasiblitycut.addTerm(flatg.trav_time[i][node], beta.get(commodity).get(node));
											feasiblitycut.addTerm(-flatg.trav_time[i][node], lambda.get(commodity).get(node));
										}else if(node == flatg.C_destination.get(commodity)){
											expre.addTerm(1, psi.get(commodity));
											
											expre1.addTerm(-1, psi.get(commodity));
										
											feasiblitycut.addTerm(flatg.trav_time[i][node], psi.get(commodity));
										}
									}
								}
							}
	 					...
	 					feasiblitycut.addTerm(flatg.Ntransit_time_devation[i][node], varphi.get(i).get(j).get(r));
	 					
					    //\tau_{ijr,1}
	 					zeta.get(i).get(j).get(r).add(model.addVar(0, 1, 0.0, GRB.BINARY, "zeta," + i+","+ node +"," + r+",1"));
						...
						constrain_num++;
	 					feasiblitycut.addTerm(-rs_index, zeta.get(i).get(j).get(r).get(0));
	 					feasiblitycut.addTerm(-rs_index, zeta.get(i).get(j).get(r).get(1));
	 				}else {
		...
		model.setObjective(feasiblitycut, GRB.MAXIMIZE);//maximize the objective
	}


	public void solve1(int timelimit) throws GRBException {
		System.out.println("solve RPtraveltime1");
		model.set(GRB.DoubleParam.MIPGap, 0);
		model.set(GRB.IntParam.Threads, 1);
		model.set(GRB.DoubleParam.BestObjStop, 0.0001);
		...
		System.out.println("objective_value " + objective_value + " " + model.get(GRB.DoubleAttr.ObjBound));
		feasibleM = false;
	}
}
