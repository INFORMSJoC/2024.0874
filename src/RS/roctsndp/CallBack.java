...
public class CallBack extends GRBCallback {
	...
	public double UB_limit = 0;
	public CallBack() {
	  }
	public void clear(double UB, double UB_l) {
		objbst = -1;
		gap_r = -1;
		startfeasible = true;
		UB_soluvalue = UB;
	...
	protected void callback() {
		try {
			if (where == GRB.CB_MIPSOL) {
				...
				if(Double.compare(objbound, 0.0000001)<=0 && Double.compare(getDoubleInfo(GRB.CB_MIPSOL_OBJBST), UB_limit)<=0) {
    				System.out.println(getDoubleInfo(GRB.CB_MIPSOL_OBJBST) +" "+UB_limit);
					abort();
    			}
	    	}
			...
			if (where == GRB.CB_MIP) {
    			...
    			if(Double.compare(objbound, 0.0000001)<=0 && Double.compare(getDoubleInfo(GRB.CB_MIP_OBJBST), UB_limit)<=0) {
    				System.out.println(getDoubleInfo(GRB.CB_MIP_OBJBST) +" "+UB_limit);
    				abort();
    			}
	    	}
			if (where == GRB.CB_MIPNODE) {
				...
				if(Double.compare(objbound, 0.0000001)<=0 && Double.compare(getDoubleInfo(GRB.CB_MIPNODE_OBJBST), UB_limit)<=0) {
					System.out.println(getDoubleInfo(GRB.CB_MIPNODE_OBJBST) +" "+UB_limit);
					abort();
    			}
	    	}
